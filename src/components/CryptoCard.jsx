         ...
         <div className="crypto-card">
            ...
            <div className="crypto-price">
                ...
                <p className={`change ${crypto.price_change_percentage_24h >= 0 ? "positive" : "negative"}`}>
                    {`${crypto.price_change_percentage_24h >= 0 ? " ↑ " : " ↓ "}`}{" "}{Math.abs(crypto.price_change_percentage_24h).toFixed(2)}%</p>
            </div>
            <div className="crypto-stats">
                <div className="stat">
                    <div className="stat-label">Total Volume</div>
                    <div className="stat-value">{formatMarketCap(crypto.total_volume)}</div>

                </div>
                <div className="stat">
                    <div className="stat-label">Circulating Supply </div>
                    <div className="stat-value">{formatMarketCap(crypto.circulating_supply)}</div>

                </div>
                <div className="stat">
                    <div className="stat-label">Market Cap</div>
                    <div className="stat-value">{formatMarketCap(crypto.market_cap)}</div>
